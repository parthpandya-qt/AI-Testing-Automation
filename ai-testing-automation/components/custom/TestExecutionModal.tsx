...
// Local TestCase type (subset) to avoid cross-file type mismatch
type TestCase = {
  id: number;
  title: string;
  description?: string;
...
type RunResult = {
  testCaseId: number;
  status: "generating" | "running" | "passed" | "failed" | "idle";
  logs: string[];
  error?: string;
  sessionId?: string;
  sessionUrl?: string;
    ...
    if (isOpen && testCases.length > 0) {
      ...
      testCases.forEach((tc) => {
        ...
        initial[tc.id] = {
          ...
          browserbaseScript: (tc as any).browserbaseScript || undefined,
          sessionId: (tc as any).session_id || (tc as any).sessionId || undefined,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-0 gap-0 bg-white rounded-2xl shadow-2xl border overflow-hidden select-none">
          ...
          {showOptions && (
            <div className="pt-3 border-t border-gray-200/60 grid grid-cols-1 md:grid-cols-3 gap-5 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="md:col-span-1 space-y-1.5">
                ...
                <div className="grid grid-cols-2 bg-gray-200/50 p-1 rounded-lg border border-gray-200">
                  <button
                    type="button"
                    disabled={isExecuting}
                  ...
                  >
                    <Terminal className="h-3.5 w-3.5" /> Run Cached
                  </button>
                  <button
                    type="button"
                    disabled={isExecuting}
                ...
                <div className="p-4 border-b bg-gray-50/50 flex justify-between items-start gap-4 shrink-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Expected: <span className="text-gray-700 font-medium">{currentSelectedTestCase.expectedResult}</span>
                    </p>
                  </div>
                ...
                <div className="flex-1 flex flex-col p-4 gap-4 overflow-y-auto">
                  ...
                  {currentSelectedResult?.browserbaseScript && (
                    <div className="border rounded-lg border-gray-200 overflow-hidden">
                      <div className="bg-gray-100 px-3 py-2.5 border-b flex items-center justify-between">
                  ...
                  <div className="flex-1 flex flex-col rounded-lg border overflow-hidden min-h-48">
                    ...
                    <div className="flex-1 p-3 bg-gray-950 font-mono text-[11px] text-gray-300 overflow-y-auto flex flex-col gap-1.5 select-text">
                      {currentSelectedResult?.logs.map((log, idx) => {
                        ...
                        const isError = log.startsWith("[SYSTEM ERROR]");
                        const isBrowser = log.startsWith("[BROWSER]");
        ...
        {/* Footer Controls */}
        <div className="border-t pt-4 flex justify-end gap-3 shrink-0 p-4">
